...
public partial class Aboutus : System.Web.UI.Page
{
    ...
    private void findImage()
    {
        SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["db"].ConnectionString);
        con.Open();
        SqlCommand cmd = new SqlCommand("select * from EventInfo where EventId =@username ", con);
        cmd.Parameters.AddWithValue("@username", Session["EventId"]);
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            lblEvent.Text = dt.Rows[0]["AboutEvent"].ToString();
            lblPrice.Text = dt.Rows[0]["Price"].ToString();
            lblEname.Text = dt.Rows[0]["EventService"].ToString();

            Session["EventService"] = dt.Rows[0]["EventService"].ToString();
